...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            var message = 'Welcome to Urban Dictionary Lookup, '
                + 'What would you like to define? For example, say define blank.';
            var reprompt = 'Ask me to define a word.';
            ...
            http.get(url, function(res) {
                ...
                res.on('end', function () {
                    ...
                    if(jsonBody.result_type == 'no_results') {
                        message = "Sorry, I could not find the definition of " + searchTerm;
                    } else {
                        var definition = jsonBody.list[0].definition;
                        var example = jsonBody.list[0].example;

                        message = searchTerm +  '. Definition: ' + definition + '. An example used in proper context is, ' + example;
                    }    
                });

            }).on('error', function (e) {
                console.log("Got error: ", e);
                message = 'Cannot reach Urban Dictionary at this time. Please try again later.';
        ...
        'Unhandled' : function () {
            var message = 'Sorry, I could not understand. Please ask me to define a word.';
            this.response.speak(message).listen(message);
            this.emit(':responseReady');
        }
    }),
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
         ...
         */
        'LaunchRequest' : function () {
            var message = 'You were listening to ' + [this.attributes['playOrder'][this.attributes['index']]].title +
                ' Would you like to resume?';
